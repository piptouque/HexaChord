...
//				PitchSetStream col_stream = MidiParser.stream_generator(_midi_file_player.getSequenceFromFile(file), _sustain_ON);
//				//col_stream.print_pcs_size_info();
//				System.out.println("proportion d'accords � au moins 3 PC : "+col_stream.get_minmum_sized_proportion_pcs(3));
//				System.out.println("proportion d'accords � au moins 4 PC : "+col_stream.get_minmum_sized_proportion_pcs(4));
//			}
//		}
//		
		//System.exit(0);
		...
		for (File file : dir.listFiles()){
			if (FileUtils.is_midi(file.toString())){
				System.out.println("file : "+file);
				PitchSetStream col_stream = MidiParser.stream_generator(_midi_file_player.getSequenceFromFile(file), _sustain_ON);
				if (col_stream.get_minmum_sized_proportion_pcs(_COMPACTNESS_DIMENSION+1) >= _MINIMUM_PERCENT_SIZED_CHORDS){
					float[] compactness_vector;
					ArrayList<TIChordComplex> complex_list = TIChordComplex.getZ12Tonnetz_n_ChordComplexList(3); 
					//ArrayList<float[]> compliance_table = col_stream.z12HexaTonnetz_compliance_table(_COMPACTNESS_DIMENSION,complex_list,_ABSOLUTE_COMPLIANCE);
					ArrayList<float[]> compliance_table = col_stream.Z12FoldedTonnetz_compliance_table(FileUtils.get_real_name(file),_COMPACTNESS_DIMENSION,complex_list,_ABSOLUTE_COMPLIANCE);
					//compactness_vector = col_stream.z12HexaTonnetz_compliance_average_table(compliance_table);
					compactness_vector = col_stream.z12FoldedTonnetz_compliance_average_table(compliance_table,_COMPACTNESS_DIMENSION);
					FileWithCompliance file_with_compliance = new FileWithCompliance(FileUtils.get_real_name(file),compactness_vector);
					file_table.add(file_with_compliance);
					System.out.println("compactness vector : "+Table.toString(compactness_vector));					
				} else {
					System.out.println("not enough "+(_COMPACTNESS_DIMENSION+1)+"chords to compute "+_COMPACTNESS_DIMENSION+"-compactness");
				}
			}
		}
...
//				System.out.println("file : "+file);
//				float[] compactness_vector;
//				PitchSetStream col_stream = MidiParser.stream_generator(_midi_file_player.getSequenceFromPath("/"+pp+"/dendro/"+file), _sustain_ON);
//				ArrayList<float[]> compliance_table = col_stream.z12HexaTonnetz_compliance_table(_COMPACTNESS_DIMENSION);
//				compactness_vector = col_stream.z12HexaTonnetz_compliance_average_table(compliance_table);
//				FileWithCompliance file_with_compliance = new FileWithCompliance(file,compactness_vector);
//				file_table.add(file_with_compliance);
//				System.out.println("compactness vector : "+Table.toString(compactness_vector));
//			}
//		}
...
//		
//		System.out.println("Table de distance par rapport � "+file_table.get(0)._name);
//		for (FileWithCompliance file_with_compliance : file_table){
//			tree_map.put(get_file_distance(file_table.get(0),file_with_compliance), file_with_compliance._name);
//		}
