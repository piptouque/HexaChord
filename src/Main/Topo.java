...
public class Topo {
	...
	public static long shorter_absolute_duration(ArrayList<PitchSetStream> stream_list) {
		...
		for (int i = 1; i < stream_list.size(); i++) {
			...
			if (stream_duration < shorter_duration)
				shorter_duration = stream_duration;
		}
	...
	public static void reduce_duration(XYSeries[] series_table, long duration_max) {
		for (XYSeries serie : series_table) {
			...
			System.out.println("Serie size : " + serie.getItemCount() + " index : " + index + " item count : "
					+ serie.getItemCount());
		}
	}

	public static void display_diagram(ArrayList<PitchSetStream> stream_list, XYSeries[] series_table, String x_axis,
			String y_axis) {
		if (MAX_DURATION) {
			if (ABSOLUTE_WINDOW) {
	...
	public static void display_dimension(ArrayList<PitchSetStream> stream_list) {
		...
		for (int i = 0; i < stream_list.size(); i++) {
			if (ABSOLUTE_WINDOW) {
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_dimension_absolute(WINDOW_SIZE,RAFFINEMMENT,file_names.get(i));
				series_table[i] = stream_list.get(i).get_xy_serie_dimension_absolute(ABSOLUTE_WINDOW_SIZE,
						ABSOLUTE_RAFFINEMMENT, stream_list.get(i).get_name());
			} else {
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_dimension_absolute(WINDOW_SIZE,RAFFINEMMENT,file_names.get(i));
				series_table[i] = stream_list.get(i).get_xy_serie_dimension_relative(RELATIVE_WINDOW_SIZE,
						RELATIVE_RAFFINEMMENT, stream_list.get(i).get_name());
			}
		}
		if (ABSOLUTE_WINDOW) {
	...
	public static void display_dimension_averages(ArrayList<PitchSetStream> stream_list) {
		...
		if (MAX_WINDOW) {
			...
			for (int i = 0; i < stream_list.size(); i++) {
				series_table[i] = stream_list.get(i)
						.get_xy_serie_dimension_realtive_average(stream_list.get(i).get_name(), window_max);
			}
		} else {
			for (int i = 0; i < stream_list.size(); i++) {
				series_table[i] = stream_list.get(i)
						.get_xy_serie_dimension_realtive_average(stream_list.get(i).get_name());
			}
		}
	...
	public static void display_betti(ArrayList<PitchSetStream> stream_list, int betti) {
		...
		for (int i = 0; i < stream_list.size(); i++) {
			if (ABSOLUTE_WINDOW) {
				series_table[i] = stream_list.get(i).get_xy_serie_betti_absolute(ABSOLUTE_WINDOW_SIZE,
						ABSOLUTE_RAFFINEMMENT, file_names.get(i), betti);
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_connexity_absolute(ABSOLUTE_WINDOW_SIZE,ABSOLUTE_RAFFINEMMENT,stream_list.get(i).get_name());
			} else {
				series_table[i] = stream_list.get(i).get_xy_serie_betti_absolute(RELATIVE_WINDOW_SIZE,
						RELATIVE_RAFFINEMMENT, file_names.get(i), betti);
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_connexity_relative(RELATIVE_WINDOW_SIZE,RELATIVE_RAFFINEMMENT,stream_list.get(i).get_name());
			}
		}
		if (ABSOLUTE_WINDOW) {
	...
	public static void display_betti_averages(ArrayList<PitchSetStream> stream_list, int betti) {
		...
		if (MAX_WINDOW) {
			int window_max = shorter_relative_duration(stream_list);
			for (int i = 0; i < stream_list.size(); i++) {
				series_table[i] = stream_list.get(i).get_xy_serie_betti_realtive_average(stream_list.get(i).get_name(),
						window_max, betti);
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_betti_realtive_average(file_names.get(i),window_max,
				// betti);
			}
		} else {
			for (int i = 0; i < stream_list.size(); i++) {
	...
	public static void display_size(ArrayList<PitchSetStream> stream_list, int dimension) {
		...
		for (int i = 0; i < stream_list.size(); i++) {
			if (ABSOLUTE_WINDOW) {
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_size_absolute(WINDOW_SIZE,RAFFINEMMENT,file_names.get(i),dimension);
				series_table[i] = stream_list.get(i).get_xy_serie_size_absolute(ABSOLUTE_WINDOW_SIZE,
						ABSOLUTE_RAFFINEMMENT, stream_list.get(i).get_name(), dimension);
			} else {
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_size_relative(WINDOW_SIZE,RAFFINEMMENT,file_names.get(i),dimension);
				series_table[i] = stream_list.get(i).get_xy_serie_size_relative(RELATIVE_WINDOW_SIZE,
						RELATIVE_RAFFINEMMENT, stream_list.get(i).get_name(), dimension);
			}
		}
		if (ABSOLUTE_WINDOW) {
	...
	public static void display_size_averages(ArrayList<PitchSetStream> stream_list, int dimension) {
		...
		if (MAX_WINDOW) {
			...
			for (int i = 0; i < stream_list.size(); i++) {
				// series_table[i] =
				// stream_list.get(i).get_xy_serie_size_relative_average(stream_list.get(i).get_name(),dimension,
				// window_max);
				series_table[i] = stream_list.get(i).get_xy_serie_size_relative_average(file_names.get(i), dimension,
						window_max);
			}
		} else {
			for (int i = 0; i < stream_list.size(); i++) {
				series_table[i] = stream_list.get(i).get_xy_serie_size_relative_average(stream_list.get(i).get_name(),
						dimension);
			}
		}
	...
	public static void display_size_averages(ArrayList<PitchSetStream> stream_list) {
		for (int d = 0; d <= 3; d++)
			display_size_averages(stream_list, d);
	}
		...
		for (int i = 0; i < stream_list.size(); i++) {
			...
			for (int j = i + 1; j < stream_list.size(); j++) {
				...
				if (complex1.is_same_complex_structure(complex2)) {
					...
					System.out.println(stream_list.get(i).get_name() + " et " + stream_list.get(j).get_name()
							+ " = MEME COMPLEXES");
				} else {
					// System.out.println(stream_list.get(i).get_name()+" et
					// "+stream_list.get(j).get_name()+" = COMPLEXES DIFFERENTS");
				}
			}
		}
	}
			...
			for (int j = 0; j < stream_list.size(); j++) {
				if (i != j) {
					...
					if (mapping_set.size() > 0) {
						System.out
								.println(stream_list.get(i).get_name() + " contient " + stream_list.get(j).get_name());
						construct_mid_files(j, i, stream_list.get(j), mapping_set);
					} else {
						// System.out.println(stream_list.get(i).get_name()+" ne contient pas
						// "+stream_list.get(j).get_name());
					}
				}
			}
		}
	...
	public static void construct_mid_files(int index_transformed, int index_support, PitchSetStream stream,
			HashSet<PitchClassMappingList> mapping_set) {

		int i = 0;
		for (PitchClassMappingList mapping : mapping_set) {
			String trans_name = midi_file_list.get(index_transformed).getName() + "_dans_"
					+ midi_file_list.get(index_support).getName() + i;
			PitchClassMappingTree pc_mapping_tree = new PitchClassMappingTree(stream.keySet(), mapping);
			TransformedSequence new_sequence = TransformedSequence
					.transforme_with_pc_mapping(get_sequence(index_transformed), pc_mapping_tree, trans_name);
	...
	public static void main(String[] args) {
		...
		for (File file : midi_file_list) {
			...
			PitchSetStream stream = MidiParser.stream_generator(_midi_file_player.getSequenceFromFile(file), sustain_ON)
					.rounded_ColStream(6);
		...
		// ChordComplex complex1 = new
		// ChordComplex(stream_list.get(0).to_pc_set_list());
		// ChordComplex complex2 = new
		// ChordComplex(stream_list.get(1).to_pc_set_list());
		//
