...
public class PosSquareGridCoordPath extends SquareGridCoordPath {
	...
	// // renvoie un CoordSet pass ou futur de p positions
	...
	// // Si la cl n'existe pas, on impose la cl infrieur la plus leve ( la plus
	// proche dans le pass)
	...
	public void inc_current_key() {
		if (higherKey(_current_key) != null)
			_current_key = higherKey(_current_key);
	}

	public void dec_current_key() {
		if (lowerKey(_current_key) != null)
			_current_key = lowerKey(_current_key);
	}
	...
	// NOT YET IMPLEMENTED BECAUSE CoordSet has no duration (contrary to
	// PitchSetWithDuration)

	// public PosTonnetzCoordPath rounded_PosTonnetzCoordPath(int r){
	// PosTonnetzCoordPath tonnetz_coord_path = new
	// PosTonnetzCoordPath(rounded_ColStream(r));
