...
//			System.out.println("Voici le coef : "+1/SimplicialTonnetz.getZ12HexaTonnetz_compliance_coef(SimplicialTonnetz.getZ12HexaTonnetzList().get(i)));
			// Compliance corrigŽe, on multiplie toutes les moyennes par le coef qui fait que l'alŽatoire tend vers1. Puis on retire 1 pour avoir la diff par rapport ˆ l'alŽatoire.
			corrected_average_table[i]=average_table[i]*Z12PlanarUnfoldedTonnetz.getZ12HexaTonnetz_compliance_coef(Z12PlanarUnfoldedTonnetz.getZ12TriangularUnfoldedTonnetzList().get(i));
			// Compliance corrigŽe, on se contente de retirer sa valeur moyenne ˆ caque myenne de compliance. (On ne normalise pas pour les rendre tous Žquitables)
			corrected_average_table[i]=average_table[i]-(1/Z12PlanarUnfoldedTonnetz.getZ12HexaTonnetz_compliance_coef(Z12PlanarUnfoldedTonnetz.getZ12TriangularUnfoldedTonnetzList().get(i)));
...
//	public void Z12HexaTonnetz_compliance_averages_display(ArrayList<ArrayList<float[]>> compliance_table, String name, int compactness_dimension) {
//		//float average_table[] = Z12HexaTonnetz_compliance_average_table(compliance_table);
//		float average_table[] = Z12HexaTonnetz_compliance_corrected_average_table(compliance_table);
...
//		}
//		PieDiagram diagram = new PieDiagram(dataset, "Complex compactness averages : "+name);
...
//	}
	...
	public ArrayList<PitchClassSet> list_all_subsets_in_seq(){
		...
		int size_max = 0;
		for (PitchClassSet set : this) if (set.size()>size_max) size_max = set.size();
