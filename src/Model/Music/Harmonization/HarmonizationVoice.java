...
public class HarmonizationVoice extends TreeMap<Long, PitchWithCompliance> {
	...
	public HarmonizationVoice(PitchSetStream voice, HarmonizationTonnetz harmonization_tonnetz,
			PitchSetStream other_voices) {
		super();
		...
		for (int key_index = 0; key_index < key_list.size(); key_index++) {
			...
			if (voice.floorEntry(key) != null && !voice.floorEntry(key).getValue().isEmpty()) {
				...
				assert current_voice.size() == 1
						: "collection " + current_voice + " must have 1 element to be in HarmonizationVoice";
				...
				if (voice.lowerEntry(key) != null && !voice.lowerEntry(key).getValue().isEmpty()) {
					...
					horizontal_compliance = harmonization_tonnetz.get_horizontal_compliance(last_pitch,
							current_voice_pitch);
					if (other_voices.lowerEntry(key) != null && !other_voices.lowerEntry(key).getValue().isEmpty()) {
						System.out.println("mesure : " + key / 4000 + " "
								+ other_voices.floorEntry(key).getValue().print_pitch_set());

						// Une collection ne peut avoir une compliance transversale que si elle est
						// associe une transition impliquant des collections de taille gales
						ArrayList<Integer> first_list = new ArrayList<Integer>(other_voices.lowerEntry(key).getValue());
						ArrayList<Integer> second_list = new ArrayList<Integer>(
								other_voices.floorEntry(key).getValue());

						if (first_list.size() == second_list.size()) {
							transversal_compliance = harmonization_tonnetz.get_transversal_compliance(
									other_voices.lowerEntry(key).getValue(), last_pitch,
									other_voices.floorEntry(key).getValue(), current_voice_pitch);
						} else {
				...
				if (other_voices.floorEntry(key) != null && !other_voices.floorEntry(key).getValue().isEmpty()) {
					PitchSetWithDuration mixed_current_pitch_set = new PitchSetWithDuration(
							other_voices.floorEntry(key).getValue());
				...
				put(key, new PitchWithCompliance(current_voice_pitch, vertical_compliance, horizontal_compliance,
						transversal_compliance));
	...
	public float compute_vertical_compliance(long last_key) {
		...
		while (key < last_key) {
			...
			if (pitch_with_compliance != null) {
				sum_vertical_compliance = sum_vertical_compliance
						+ pitch_with_compliance.get_v_compliance() * get_pitch_duration(key);
			}
			key = higherKey(key);
		}
		sum_vertical_compliance = sum_vertical_compliance
				+ get(lastKey()).get_v_compliance() * get_pitch_duration(lastKey());
	...
	public float compute_transversal_compliance(long last_key) {
		...
		// System.out.println("tcsum : "+transversal_compliance+" moves_count :
		// "+moves_count);
		...
		// key = higherKey(key);
		// }
		// System.out.println("key : "+lastKey()+" tc :
		// "+get(lastKey()).get_t_compliance());
		//
		// }
	...
	public float get_mixed_compliance() {
		float c = _final_transversal_compliance
				* (_final_horizontal_compliance + (float) 2 * _final_vertical_compliance) / (float) 3;
		return c;
	}
