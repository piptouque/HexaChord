...
public class PitchSetStream extends TreeMap<Long, PitchSetWithDuration> {
	...
	// pitch_list_stream est quivalent au picthSetStream sauf que les pc sont
	// ordonnes.
	public SquareGridCoordPath compute_tonnetz_coord_path(PlanarUnfoldedTonnetz t,
			TreeMap<Long, ArrayList<Integer>> pitch_list_stream_in_t) {
	...
	// public int get_new_pitch(int old_pitch, long key, TonnetzCoordPath new_path,
	// PlanarUnfoldedTonnetz t){
	...
	//// System.out.println("old_pitch : "+old_pitch+" old_path_pitch :
	// "+t.xy_coords_to_pitch(old_coord.get(i)));
	// if (t.xy_coords_to_pitch(old_coord.get(i))==(old_pitch%12)){
	//// System.out.println("old_pitch : "+old_pitch+" new pitch returned :
	// "+(60+t.xy_coords_to_pitch(new_path.get(key).get(i))));
	...
	// Si le tick demand tombe entre 2 collections, c'est la dernire collection (la
	// plus proche mais dans le pass) qui est retourne
	...
	public void insert_col(Map.Entry<Long, PitchSetWithDuration> entry) {
		...
		// 1er cas : La collection dbute et finit des dates dj frontires dans le Stream.
		...
		if (containsKey(tick) && containsKey(tick + col.get_duration())) {
			...
			// 2nd cas : La collection ne dbute pas une date frontire dans le Stream
			if (!containsKey(tick)) {
				long end = lastKey() + get(lastKey()).get_duration();
				// Cas 2A : La collection dbute aprs la fin du stream. Il faut rallonger le
				// stream. En principe a n'est pas possible, toutes les pistes sont de mme
				// longueur. En fait si car quand a finit par du vide ce n'est pas la peine de
				// crer un long MusicCollection vide.
				if (tick > end) {
					PitchSetWithDuration new_c = new PitchSetWithDuration(tick + col.get_duration() - end);
					put(end, new_c);
					insert_col(entry);
				}
				// Cas 2B : La collection dbute avant la fin du stream
				else {
					// Cas 2B1 : La collection dbute avant le dbut du stream
					if (tick < firstKey()) {
						PitchSetWithDuration new_c = new PitchSetWithDuration(firstKey() - tick);
						put(tick, new_c);
						insert_col(entry);
						// Cas 2B2 : La collection dbute entre le dbut et la fin du stream
					} else {
						...
						TreeMap<Long, PitchSetWithDuration> tail = new TreeMap<Long, PitchSetWithDuration>(
								tailMap(prev_key));
						long next_key;
						if (tail.size() == 1) {
							next_key = end;
						...
						// Cas 2B1 : La collection est l'intrieur des frontires d'une unique collection
						// du stream
						if (next_key > tick + col.get_duration()) {
							...
							// CAS 2B2 : La collection ne dbute pas une date frontire dans le Stream mais
							// elle arrive au moins la frontire suivante
							// On raffine le stream de manire se rediriger vers le cas 1
						} else {
							PitchSetWithDuration new_c = new PitchSetWithDuration(get(prev_key));
			...
			// 3eme cas : La collection dbute une date frontire mais ne finit pas une date
			// frontire
			// A la fin du if on re-rentre dans la fonction de manire rcursive
			else {
				...
				if ((tick + col.get_duration()) <= end) {
					...
					long next_key; // next_key : date dans le stream la plus proche "aprs" la fin de la collection
					// Si la collection s'achve au cours de la dernire collection du stream
					if (prev_key == lastKey()) {
						next_key = end;
					} else {
						TreeMap<Long, PitchSetWithDuration> tail = new TreeMap<Long, PitchSetWithDuration>(
					...
					// CAS 3B : la collection finit aprs la fin du stream (Il faut rallonger le
					// stream)
					PitchSetWithDuration new_c = new PitchSetWithDuration((tick + col.get_duration()) - end);
					put(end, new_c);
					insert_col(entry);
	...
	public void struct_verification() {
		...
		if (!this.isEmpty()) {
			...
			for (Long key : key_set) {
				...
				assert key == last_key + last_duration || key == firstKey()
						: "ColStream Structure Verification Error on key " + key;
	...
	//// System.out.println(key_list.get(n)+" "+key_list.get(n+1)+"
	// "+get(key_list.get(n)));
	...
	// put(key_list.get(n),new
	// PitchSetWithDuration(get(key_list.get(n)).get_duration()+get(key_list.get(n+1)).get_duration()));
	// remove(key_list.get(n+1));
	//
	// bool=false;
	// }
		...
		while (n < key_list.size() - 2 && bool) {
			...
			if (!get(key_list.get(n)).isEmpty()) {
				...
				if (get(key_list.get(n + 1)).isEmpty()) {
					put(key_list.get(n), new PitchSetWithDuration(
							get(key_list.get(n)).get_duration() + get(key_list.get(n + 1)).get_duration()));
					remove(key_list.get(n + 1));
					return true;
				}
			}
	...
	// Supress empty collections at the beginning of the stream (Warning : Other
	// collections onsets are all shifted).
	// This make a desynchronisation with the original midi file so don't use with
	// midi player !
	...
	// Construction d'une PosColStream filtrant uniquement les collections de dure
	// suprieure r (par exemple r = 10 miditicks)
	// Les dures des collections conserves sont agrandies (on ajoute les dures des
	// rsidus) pour prserver la dure totale du fichier
	...
	public PitchSetStream rounded_ColStream(int r) {
		...
		for (int i = 1; i < key_list.size(); i++) {
			...
			if (collection.get_duration() < r) { // si la collection est très courte
				...
				// collection.
				// if (!last_col.containsAll(collection) && rounded_onset_list.contains(key)){
				// // si elle fait apparaitre au moins un nouveau pitch (ie elle est déclenchée
				// par au moins un offset) ET que key appartient à la liste d'onsets
				// approximatifs
				// stream.put(key, collection);
				// } else { // si elle ne fait apparaitre aucun nouveau pitch (ie elle est
				// uniquement déclenchée par un (des) offset) ou si il y a un onset mais pas
				// dans la liste des onsets approximatifs
	...
	public List<Long> get_onset_list() {
		...
		for (int i = 0; i < key_list.size(); i++) {
			...
			if (key == firstKey()) {
				...
				if (!prev_collection.containsAll(collection))
					onset_list.add(key);
			}
		}
		return onset_list;
	...
	public List<Long> get_rounded_onset_list(int r) {
		...
		for (int i = 1; i < onset_list.size(); i++) {
			...
			if (current_onset - last_round_onset >= r)
				rounded_onset_list.add(current_onset);
		}
	...
	public static void compliance_display(ArrayList<PitchSetStream> stream_list, ArrayList<TIChordComplex> complex_list,
			int compactness_dimension) {

		ArrayList<float[]> average_table_list = new ArrayList<float[]>();
		for (PitchSetStream stream : stream_list) {
			average_table_list
					.add(stream.z12FoldedTonnetz_compliance_average_table(stream.Z12FoldedTonnetz_compliance_table(
							stream.get_name(), compactness_dimension, complex_list, false), compactness_dimension));
		}
		...
		for (int i = 0; i < average_table_list.get(0).length; i++) {
			for (int j = 0; j < average_table_list.size(); j++) {
				...
				// if (average_table_list.get(j)[i]!=0){ // pour ne pas tracer les espaces dont
				// la compliance = 0
				// dataset.setValue(complex_names.get(i), average_table[i]);
				h_dataset.setValue(average_table_list.get(j)[i], stream_list.get(j).get_name(),
						complex_list.get(i).toString());

				// h_dataset.setValue(average_table[i], name, complex_list.get(i).toString());
				// h_dataset.setValue(complex_list.get(i).get_average_compactness(12,
				// compactness_dimension), "random chords", complex_list.get(i).toString());
				// }
			}
		}
		...
		if (compactness_dimension == -1)
			compactness_type = "abs" + "-compactness";
		else
			compactness_type = compactness_dimension + "-compactness";
		bar_diagram.display_bar_diagram(compactness_type);

	}

	public void compliance_averages_compare_random_display(ArrayList<float[]> compliance_table,
			ArrayList<TIChordComplex> complex_list, String name, int compactness_dimension) {
		...
		for (int i = 0; i < average_table.length; i++) {
			// if (average_table[i]!=0){ // pour ne pas tracer les espaces dont la
			// compliance = 0
			// dataset.setValue(complex_names.get(i), average_table[i]);
			h_dataset.setValue(average_table[i], name, complex_list.get(i).toString());
			h_dataset.setValue(complex_list.get(i).get_average_compactness(12, compactness_dimension), "random chords",
					complex_list.get(i).toString());
			// }
		}
	...
	public void Z12FoldedTonnetz_compliance_display(String name, int compactness_dimension,
			ArrayList<TIChordComplex> complex_list, boolean compare_to_random) {
		ArrayList<float[]> compliance_list = Z12FoldedTonnetz_compliance_table(name, compactness_dimension,
				complex_list, false);
		compliance_display(name, compliance_list, complex_list, compactness_dimension);
	}

	public void compliance_display(String name, ArrayList<float[]> compliance_list,
			ArrayList<TIChordComplex> complex_list, int compactness_dimension) {

		XYSeries[] series_table = new XYSeries[complex_list.size()];
		for (int i = 0; i < complex_list.size(); i++)
			series_table[i] = new XYSeries(complex_list.get(i).toString());
		int j;
		...
		// compliance_averages_display(compliance_list,
		// TIChordComplex.get_complex_string_list(complex_list), name,
		// compactness_dimension);
		compliance_averages_compare_random_display(compliance_list, complex_list, name, compactness_dimension);

	}

	// Compliance Table : Pour chaque collection du stream on dfinit le tableau ||
	// Index | Date_dbut | dure | c_T[1] | ... | c_T[1,2,3,4,5,6] ||
	public ArrayList<float[]> Z12FoldedTonnetz_compliance_table(String name, int compactness_dimension,
			ArrayList<TIChordComplex> complex_list, boolean absolute_compliance) {
		ArrayList<float[]> compliance_table = new ArrayList<float[]>();
		ArrayList<Long> key_list = new ArrayList<Long>(keySet());

		for (int i = 0; i < key_list.size(); i++) {
			// Pour chaque collection du stream on dfinit le tableau || Index | Date_dbut |
			// dure | c_T[1,1,10] | ... | c_T[4,4,4] ||
			float[] collection_compliance = new float[complex_list.size() + 3];
			collection_compliance[0] = i; // Index
			collection_compliance[1] = key_list.get(i); // Date_dbut
			collection_compliance[2] = get(key_list.get(i)).get_duration(); // dure
			for (int j = 0; j < complex_list.size(); j++) {
				if (absolute_compliance)
					...
					collection_compliance[j + 3] = complex_list.get(j).get_n_compliance(get(key_list.get(i)),
							compactness_dimension);
				}
			}
	...
	public void compliance_averages_display(ArrayList<float[]> compliance_table, ArrayList<String> complex_names,
			String name, int compactness_dimension) {
		float average_table[] = z12FoldedTonnetz_compliance_average_table(compliance_table, compactness_dimension);
		DefaultCategoryDataset h_dataset = new DefaultCategoryDataset();
	...
	// Calcul de la compliance moyenne. Le fait de moyenner la somme en la divisant
	// par la dure du morceau n'a de sens que si l'on veut comparer avec la
	// compliance d'un autre morceau.
	// Si il s'agit uniquement de comparer la compliance dans les diffrents espaces
	// pour un morceau donn a ne sert rien de diviser et on peut utiliser la
	// fonction
	// Z12HexaTonnetz_compliance_sum_table (not yet implemented)

	public float[] z12FoldedTonnetz_compliance_average_table(ArrayList<float[]> compliance_table,
			int compactness_degree) {

		// N_AVERAGING = TRUE : on normalise seulement par rapport aux moments
		// correspondant au moins N pitch class simultanes.
		...
		for (float[] f : compliance_table) {
			...
			for (int i = 0; i < complex_count; i++) {
				// La moyenne prend compte des compliances pondres par la dure de la collection
				// concerne
				average_table[i] = average_table[i] + (f[i + 3] * f[2]);
				// La moyenne prend compte des compliances NON pondres par la dure de la
				// collection concerne
	...
	public void absolute_compliance_display(String name, ArrayList<float[]> compliance_list,
			ArrayList<TIChordComplex> tonnetz_list) {
		XYSeries[] series_table = new XYSeries[tonnetz_list.size()];
		for (int i = 0; i < tonnetz_list.size(); i++)
			series_table[i] = new XYSeries(tonnetz_list.get(i).toString());
		int j;
		for (int i = 0; i < compliance_list.size(); i++) {
			float[] f = compliance_list.get(i);
		...
		// On cr une nouvelle liste avec seulement les tonnetz utiles pour arer le
		// digrame
		...
		for (XYSeries xyserie : series_table) {
			...
			for (int i = 0; i < array[0].length; i++) {
				if (array[1][i] != 0)
					useless_tonnetz = false;
			}
			if (!useless_tonnetz)
				new_list.add(xyserie);

		}

		XYSeriesCollection dataset = new XYSeriesCollection();
		;
		for (XYSeries s : new_list)
			dataset.addSeries(s);
	...
	/* I Tonnetze hxagonaux */

	// public void Z12HexaTonnetz_compliance_display(String name, int
	// compactness_dimension) {
	// ArrayList<TonnetzChordComplex> complex_list =
	// TonnetzChordComplex.getZ12Tonnetz_n_ChordComplexList(3);
	// ArrayList<float[]> compliance_list =
	// Z12FoldedTonnetz_compliance_table(compactness_dimension,complex_list,false);
	//
	//
	// XYSeries[] series_table = new XYSeries[complex_list.size()];
	// for (int i =0;i<complex_list.size();i++) series_table[i] = new
	// XYSeries(complex_list.get(i).toString());
	// int j;
	// for (int i=0;i<compliance_list.size();i++){
	// float[] f = compliance_list.get(i);
	// for (j=0;j<complex_list.size();j++){
	...
	//
	//// z12HexaTonnetz_compliance_averages_display(compliance_list, complex_list,
	// name, compactness_dimension);
	// compliance_averages_display(compliance_list,
	// TonnetzChordComplex.get_complex_string_list(complex_list), name,
	// compactness_dimension);
	//
	// }
	//

	// public float[] z12HexaTonnetz_compliance_average_table(ArrayList<float[]>
	// compliance_table) {
	//
	// // N_AVERAGING = TRUE : on normalise seulement par rapport aux moments
	// correspondant au moins N pitch class simultanes.
	...
	// //for (int i = 0;i<12;i++){
	// for (int i = 0;i<complex_count;i++){
	// // La moyenne prend compte des compliances pondres par la dure de la
	// collection concerne
	// average_table[i]=average_table[i]+(f[i+3]*f[2]);
	// // La moyenne prend compte des compliances NON pondres par la dure de la
	// collection concerne
	...
	// Compliance Table : Pour chaque collection du stream on dfinit le tableau ||
	// Index | Date_dbut | dure | c_T[1,1,10] | ... | c_T[4,4,4] ||
	// public ArrayList<float[]> z12HexaTonnetz_compliance_table(int
	// compactness_dimension, ArrayList<TonnetzChordComplex> complex_list, boolean
	// absolute_compliance){
	// ArrayList<float[]> compliance_table = new ArrayList<float[]>();
	// ArrayList<Long> key_list = new ArrayList<Long>(keySet());
	//
	...
	// collection_compliance[2] = get(key_list.get(i)).get_duration(); // dure
	// //for (int
	// j=0;j<TonnetzChordComplex.getZ12Tonnetz_3_ChordComplexList().size();j++){
	// //for (int
	// j=0;j<TonnetzChordComplex.getZ12Tonnetz_n_ChordComplexList(3).size();j++){
	// for (int j=0;j<complex_list.size();j++){
	...
	// public void z12HexaTonnetz_compliance_averages_display(ArrayList<float[]>
	// compliance_table, ArrayList<TonnetzChordComplex> complex_list, String name,
	// int compactness_dimension) {
	// //float average_table[] =
	// z12HexaTonnetz_compliance_average_table(compliance_table);
	// float average_table[] =
	// z12FoldedTonnetz_compliance_average_table(compliance_table,compactness_dimension);
	//
	...
	// //dataset.setValue(TonnetzChordComplex.getZ12Tonnetz_3_ChordComplexList().get(i).toString(),
	// average_table[i]);
	// dataset.setValue(TonnetzChordComplex.getZ12Tonnetz_n_ChordComplexList(3).get(i).toString(),
	// average_table[i]);
	// //h_dataset.setValue(average_table[i], "",
	// TonnetzChordComplex.getZ12Tonnetz_3_ChordComplexList().get(i).toString());
	// h_dataset.setValue(average_table[i], "",
	// TonnetzChordComplex.getZ12Tonnetz_n_ChordComplexList(3).get(i).toString());
	// }
	// PieDiagram diagram = new PieDiagram(dataset, "Complex compliance averages :
	// "+name);
	...
	// public void Z12FoldedTonnetz_compliance_display(String name, int n,
	// ArrayList<Z12FoldedChordGraphTonnetz> tonnetz_list){
	// ArrayList<float[]> compliance_list = Z12FoldedTonnetz_compliance_table(n,
	// tonnetz_list, false);
	...
	/*
	 * Enumration des intervales MI horizontaux entre une collection et celle qui
	 * prcde Mthode "large" : on limine tous les pc commun et on prend toutes les
	 * combinaisons parmis les pitchs restant
	 */
	...
	/*
	 * Enumration des intervales MI horizontaux entre une collection et celle qui
	 * prcde Mthode "contrainte" : on limine tous les pc commun. un pitch ne peut
	 * aller qu' un unique endroit : la plus courte distance
	 */
	...
	public ArrayList<float[]> z12HexaTonnetz_large_Hcompliance_table(boolean constraint) {
		...
		for (int i = 1; i < key_list.size(); i++) {
			...
			for (int j = 0; j < Z12PlanarUnfoldedTonnetz.getZ12TriangularUnfoldedTonnetzList().size(); j++) {
				...
				collection_compliance[j + 3] = Z12PlanarUnfoldedTonnetz.getZ12TriangularUnfoldedTonnetzList().get(j)
						.Hcompliance(horizontal_interval_list);
				// System.out.println("tonnetz :
				// "+UnfoldedSimplicialTonnetz.getZ12HexaTonnetzList().get(j));
			}
			compliance_table.add(collection_compliance);
		}
		return compliance_table;
	...
	public void Z12HexaTonnetz_Hcompliance_display(String name, boolean constraint) {
		...
		ArrayList<Z12PlanarUnfoldedTonnetz> tonnetz_list = Z12PlanarUnfoldedTonnetz
		...
		for (int i = 0; i < compliance_list.size(); i++) {
			...
			for (j = 0; j < tonnetz_list.size(); j++) {
				...
				series_table[j].add(f[1] + f[2],
						f[3 + Z12PlanarUnfoldedTonnetz.getZ12HexaTonnetzIndex(tonnetz_list.get(j))]);
			}
		}
		XYSeriesCollection dataset = new XYSeriesCollection();
		;
		...
		compliance_averages_display(compliance_list, Z12PlanarUnfoldedTonnetz.get_complex_string_list(tonnetz_list),
				name, -2);
	...
	public String print_voice_compliance(PitchSetStream target_voice) {
		...
		return _name + " : target compliance : v = " + target_vertical_compliance + " h = "
				+ target_horizontal_compliance + " t = " + target_transversal_compliance;
	...
	// }else {
	// tmp_similarity =
	// get(key).get_similarity(st_target.floorEntry(key).getValue());
	// }
	...
	public ContinuePitchSpace get_choral_voice_space(int voice, PitchSetWithDuration pitch_set) {
		...
		// assert get(key).size()>=voice+1 : "not enough voices in "+get(key)+" to find
		// the space of the voice "+voice;
	...
	public HarmonizationTonnetz get_tonal_harmonization_tonnetz(Scale scale) {
		...
		horizontal_tonnetz_ranking.put((float) 0.8,
				new Z7FoldedGraphTonnetz(scale, new IntegerSet(new int[] { 2, 3 })));
	...
	public HarmonizationTonnetz get_harmonization_tonnetz() {
		...
		// vertical_tonnetz =
		// Z12FoldedGraphTonnetz.getZ12FoldedChordGraphTonnetzList().get(16);
		// vertical_tonnetz =
		// TonnetzChordComplex.getZ12Tonnetz_3_ChordComplexList().get(10);
		...
		// horizontal_tonnetz_ranking.put((float) 1, new Z12FoldedGraphTonnetz(new
		// int[]{0,1,2}));
		// horizontal_tonnetz_ranking.put((float) 0.6, new Z12FoldedGraphTonnetz(new
		// int[]{3,4,5}));
	...
	public void spatial_harmonization(PitchSetStream target_voice, int voice_number) {
		...
		// horizontal_tonnetz_ranking_test.put((float) 1, new Z12FoldedTonnetz(new
		// int[]{1,2}));
		// horizontal_tonnetz_ranking_test.put((float) 0.8, new Z12FoldedTonnetz(new
		// int[]{3,4,5}));
		//
		// System.out.println("beau tonnetz : "+Z7FoldedChordTonnetz.get_triad_strip(new
		// Scale(0,"major")));
		//
		// HarmonizationTonnetz harmonization_tonnetz_test = new
		// HarmonizationTonnetz(Z7FoldedChordTonnetz.get_triad_strip(new
		// Scale(7,"major")),horizontal_tonnetz_ranking_test);
		//
		// ColStream stream_test = new ColStream();
		//// PitchSetWithDuration set1 = new PitchSetWithDuration(new IntegerSet(new
		// int[]{60,64}),1000);
		//// PitchSetWithDuration set2 = new PitchSetWithDuration(new IntegerSet(new
		// int[]{60,64,67,71}),1000);
		//// PitchSetWithDuration set3 = new PitchSetWithDuration(new IntegerSet(new
		// int[]{60,64,67,70}),1000);
		// PitchSetWithDuration set4 = new PitchSetWithDuration(new IntegerSet(new
		// int[]{55,62,71}),1000);
		//
		...
		// "+harmonization_tonnetz_test.get_vertical_compliance(set4));
		//
		// PitchRanking pitch_ranking_test =
		// set4.get_harmo_pitch_ranking(harmonization_tonnetz_test,
		// get_choral_voice_space(voice_number,set4));
		...
		// horizontal_tonnetz_ranking.put((float)1, new Z7FoldedTonnetz(scale,new
		// IntegerSet(new int[]{0,1})));
		// horizontal_tonnetz_ranking.put((float)0.8, new Z7FoldedTonnetz(scale,new
		// IntegerSet(new int[]{2,3})));
		// } else {
		// vertical_tonnetz =
		// Z12FoldedChordTonnetz.getZ12FoldedChordTonnetzList().get(16);
		// System.out.println("vertical tonnetz : "+vertical_tonnetz);
		// horizontal_tonnetz_ranking.put((float) 1, new Z12FoldedTonnetz(new
		// int[]{0,1,2}));
		// horizontal_tonnetz_ranking.put((float) 0.6, new Z12FoldedTonnetz(new
		// int[]{3,4,5}));
		// }

		// TONAL
		// HarmonizationTonnetz harmonization_tonnetz =
		// get_tonal_harmonization_tonnetz(scale);
		// ATONAL
		HarmonizationTonnetz harmonization_tonnetz = get_harmonization_tonnetz();
		...
		float target_horizontal_compliance = target_harmo_voice.get_final_horizontal_compliance();
		float target_transversal_compliance = target_harmo_voice.get_final_transversal_compliance();
		System.out.println("target compliance : v = " + target_vertical_compliance + " h = "
				+ target_horizontal_compliance + " t = " + target_transversal_compliance);
		...
		for (long key : keySet()) {
			if (get(key).get_duration() > 5 && !get(key).isEmpty()) {
				...
				// get(key).get_harmo_pitch_ranking(vertical_Z12_tonnetz, vertical_Z7_tonnetz,
				// get_choral_voice_space(voice_number,key));
				// PitchRanking pitch_ranking =
				// get(key).get_harmo_pitch_ranking(harmonization_tonnetz, vertical_Z7_tonnetz,
				// get_choral_voice_space(voice_number,key));
				PitchRanking pitch_ranking;
				if ((key == firstKey() && !firstEntry().getValue().isEmpty())
						|| (key == higherKey(firstKey()) && firstEntry().getValue().isEmpty())
								&& first_chord_is_first_degree) {
					if (tonal && first_chord_is_first_degree) {
						pitch_ranking = get(key).get_harmo_pitch_ranking(harmonization_tonnetz,
								get_choral_voice_space(voice_number, key, false), scale.get_degree(0));
					} else {
						pitch_ranking = get(key).get_harmo_pitch_ranking(harmonization_tonnetz,
								get_choral_voice_space(voice_number, key, false));
					}

				} else {
					if (voice_can_meet_in_short_spaces) {
						pitch_ranking = get(key).get_harmo_pitch_ranking(harmonization_tonnetz,
								get_choral_voice_space(voice_number, key, true));
					} else {
						pitch_ranking = get(key).get_harmo_pitch_ranking(harmonization_tonnetz,
								get_choral_voice_space(voice_number, key, false));
					}
		...
		ArrayList<HarmonizationVoice> voice_list = dynamic_pitch_ranking.get_voice_set(harmonization_tonnetz, this,
				target_harmo_voice);
		...
		// ArrayList<HarmonizationVoice> voice_list = new
		// ArrayList<HarmonizationVoice>(voice_set);

		if (voice_list.size() < 50 && voice_list.size() > 0) {
			for (int i = 0; i < voice_list.size(); i++) {
				String name = "harmonization" + i;
				System.out.println(name + " cv = " + voice_list.get(i).get_final_vertical_compliance() + " ch = "
						+ voice_list.get(i).get_final_horizontal_compliance() + " ct = "
						+ voice_list.get(i).get_final_transversal_compliance());
		...
		// System.out.println("transversal comp :
		// "+harmonization_tonnetz.get_compliance(test_stream.get((long)0),
		// test_stream.get((long)1000)));
		...
		// System.out.println("transversal comp :
		// "+harmonization_tonnetz.get_compliance(dom,res));

		// for (HarmonizationVoice voice : voice_set){
		// ColStream reconstructed_stream = ColStream.mix_2colStream(this,
		// voice.to_ColStream());
		...
		// ArrayList<Long> key_list = new
		// ArrayList<Long>(reconstructed_stream.keySet());
		...
		// }
		// System.out.println("new hori compli "+new_hori_compli.size()+" :
		// "+new_hori_compli);
		// }
	...
	public void draw_voice_table(ArrayList<HarmonizationVoice> voice_list, HarmonizationVoice target) {
		...
		for (int i = 0; i < voice_list.size(); i++) {
			voices_series.add(voice_list.get(i).get_final_horizontal_compliance(),
					voice_list.get(i).get_final_vertical_compliance());
		}
	...
	public ChordComplex get_absolute_window_complex(long start_in_ticks, long duration_in_ticks) {
		HashSet<PitchClassSet> pcs_set = new HashSet<PitchClassSet>();
		if (start_in_ticks > get_duration())
			return null;
	...
	public XYSeries get_xy_serie_betti_absolute(long window_size_in_ms, long raffinnement_in_ms, String name,
			int betti_number) {
		return get_XYSeries_from_TreeMap(
				get_betti_over_absolute_time(window_size_in_ms, raffinnement_in_ms, betti_number), name);
	}

	public TreeMap<Double, Integer> get_betti_over_absolute_time(long window_size_in_ms, long raffinnement_in_ms,
			int betti_number) {
		...
		for (long key = window_size_in_ticks; key <= lastKey(); key = key + (1 * raffinnement_in_ticks)) {
			...
			betti_over_time.put((double) key / (1000 * get_tick_number_per_milli_second()),
					complex.get_betti_number(betti_number));
		}
	...
	public TreeMap<Double, Integer> get_dimension_over_absolute_time(long window_size_in_ms, long raffinnement_in_ms) {
		...
		for (long key = window_size_in_ticks; key <= lastKey(); key = key + (1 * raffinnement_in_ticks)) {
			ChordComplex complex = get_absolute_window_complex(key - window_size_in_ticks, window_size_in_ticks);
			dimension_over_time.put((double) key / (1000 * get_tick_number_per_milli_second()),
					complex.get_dimension());
		}
	...
	public XYSeries get_xy_serie_size_absolute(long window_size_in_ms, long raffinnement_in_ms, String name,
			int dimension) {
		// return get_xy_serie_double_key(get_size_over_absolute_time(window_size_in_ms,
		// raffinnement_in_ms, dimension),name);
		return get_XYSeries_from_TreeMap(get_size_over_absolute_time(window_size_in_ms, raffinnement_in_ms, dimension),
				name);
	}

	public TreeMap<Double, Integer> get_size_over_absolute_time(long window_size_in_ms, long raffinnement_in_ms,
			int dimension) {
		long window_size_in_ticks = (long) (get_tick_number_per_milli_second() * window_size_in_ms);
		long raffinnement_in_ticks = (long) (get_tick_number_per_milli_second() * raffinnement_in_ms);
		...
		for (long key = window_size_in_ticks; key <= lastKey(); key = key + (1 * raffinnement_in_ticks)) {
			ChordComplex complex = get_absolute_window_complex(key - window_size_in_ticks, window_size_in_ticks);
			size_over_time.put((double) key / (1000 * get_tick_number_per_milli_second()),
					complex.get_d_simplex_count(dimension));
		}
	...
	public XYSeries get_xy_serie_size_relative(int window_size, int raffinnement, String name, int dimension) {
		// return get_xy_serie_long_key(get_size_over_relative_time(window_size,
		// raffinnement, dimension),name);
	...
	// public XYSeries get_xy_serie_long_key(TreeMap<Long,Integer> tree_map, String
	// name){
	// XYSeries serie = new XYSeries(name);
	// System.out.println("name : "+name);
	// System.out.println("treemap : "+tree_map);
	// for(long key : tree_map.keySet()){
	// //serie.add(key/((long)1000*get_tick_number_per_milli_second()),
	// tree_map.get(key));
	...
	// }
	//
	// public XYSeries get_xy_serie_double_key(TreeMap<Double,Integer> tree_map,
	// String name){
	// XYSeries serie = new XYSeries(name);
	// System.out.println("name : "+name);
	// System.out.println("treemap : "+tree_map);
	...
	// //System.out.println("nombre de ticks en 1/10 sec :
	// "+(get_tick_number_per_second()/10));
	// long window_size_in_ticks =
	// (long)(get_tick_number_per_milli_second()*window_size_in_ms);
	// long raffinnement_in_ticks =
	// (long)(get_tick_number_per_milli_second()*raffinnement_in_ms);
	//
	// TreeMap<Long,Integer> dimension_over_time =
	// get_dimension_over_absolute_time(window_size_in_ticks,
	// raffinnement_in_ticks);
	// XYSeries serie = new XYSeries("dimension over time");
	// for (long key : dimension_over_time.keySet()){
	// serie.add(key/((long)1000*get_tick_number_per_milli_second()),
	// dimension_over_time.get(key));
	// }
	// XYSeriesCollection dataset = new XYSeriesCollection();
	...
	public long get_closest_key(long key) {
		...
		if (dist_to_lower_key < dist_to_higher_key)
			return lower_key;
		else
			return higher_key;
	}
	...
	// TreeMap<Long,List<HAChordCandidate>> cand_stream = new
	// TreeMap<Long,List<HAChordCandidate>>();
	// for (long key : keySet()){
	// if (!get(key).isEmpty()){
	...
	public List<Scale> get_HA_compatible_keys(long key) {
		...
		for (Scale major_scale : Scale.get_major_scales()) {
			if (major_scale.containsAll(current_pcs))
				compatible_keys.add(major_scale);
		}
		for (Scale minor_scale : Scale.get_minor_scales()) {
			if (minor_scale.containsAll(current_pcs))
				compatible_keys.add(minor_scale);
		}
		if (compatible_keys.isEmpty())
			compatible_keys = get_HA_compatible_keys(lowerKey(key));
	...
	public Set<PitchClassSet> get_HA_compatible_chords(long key) {
		...
		if (compatible_chords.isEmpty())
			compatible_chords = get_HA_compatible_chords(lowerKey(key));
