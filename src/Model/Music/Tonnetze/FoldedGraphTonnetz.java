...
public abstract class FoldedGraphTonnetz extends HashSet<Integer> {
	...
	public boolean is_clique(IntegerSet pitch_set) {
		...
		for (int i = 0; i < pitch_list.size() - 1; i++) {
			for (int j = i + 1; j < pitch_list.size(); j++) {
				if (!are_neighbor(pitch_list.get(i), pitch_list.get(j)))
					is_clique = false;
			}
		}
		return is_clique;
	...
	public IntegerSet complete_chord_triad_pc_set(PitchClassSet c) {
		...
		for (int[] cliques : get_2_cliques(c)) {
			for (int i : _scale) {
				if (!c.contains(i) && get_neighbors_pitch_class(i).contains(cliques[0])
						&& get_neighbors_pitch_class(i).contains(cliques[1])) {
					set.add(i);
				}
			}
		}
	...
	// // Fonction retournant la liste des Tonnetz voisins d'un Tonnetz selon le
	// critre :
	...
	// Z12PlanarUnfoldedTonnetz t =
	// n.get_corresponding_Tonnetz(Z12PlanarUnfoldedTonnetz.getZ12TriangularUnfoldedTonnetzList());
	// if (t!=null && t!=this) list.add(t);
	// }
	// return list;
	// }
	//
	// // Fonction retournant la liste des Tonnetz voisins dans le graphe des
	// Tonnetz selon le critre :
	// // 2 Tonnetz sont voisins si un mouvement minimal permet de passer de l'un
	// l'autre
	// public static HashSet<HashSet<Z12PlanarUnfoldedTonnetz>>
	// neighbor_list(ArrayList<Z12PlanarUnfoldedTonnetz> tonnetzs){
	// HashSet<HashSet<Z12PlanarUnfoldedTonnetz>> neighbor_list = new
	// HashSet<HashSet<Z12PlanarUnfoldedTonnetz>>();
	// for (Z12PlanarUnfoldedTonnetz t :
	// Z12PlanarUnfoldedTonnetz.getZ12TriangularUnfoldedTonnetzList()){
	// HashSet<Z12PlanarUnfoldedTonnetz> set = t.neighbor_tonnetzs();
	// for (Z12PlanarUnfoldedTonnetz n : set){
	// HashSet<Z12PlanarUnfoldedTonnetz> neighborhood = new
	// HashSet<Z12PlanarUnfoldedTonnetz>();
	// neighborhood.add(n);
	// neighborhood.add(t);
