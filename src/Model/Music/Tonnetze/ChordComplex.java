...
public class ChordComplex {
	...
	public int get_dimension(){
		...
		for (PitchClassSet pcs : _total_pcs_set){
			if (pcs.size()-1>higher_dimension) higher_dimension = pcs.size()-1; 
		}
	...
	public boolean contains_chord(PitchClassSet pcs){
		for (PitchClassSet pcs2 : get_sized_pitch_class_sets(pcs.size())){
			if (pcs2.containsAll(pcs)) return true;
		}
		return false;
	}
	...
	public HashSet<PitchClassSet> get_sized_pitch_class_sets(int size){
		...
		for (PitchClassSet pcs : _total_pcs_set){
			if (pcs.size()==size) pcs_set.add(pcs);
		}
		return pcs_set;
	}
	...
	public ChordComplex get_n_skeleton(int n){
		if (n>=get_dimension()) return this;
		HashSet<PitchClassSet> pcs_set = new HashSet<PitchClassSet>();
		for (PitchClassSet pcs : get_total_pcs_set()){
			if (pcs.size() <= n+1) pcs_set.add(pcs);
		}
	...
	public ExplicitSimplexStream get_Plex_complex(){
		...
		if (_Plex_complex != null) return _Plex_complex;
	...
	// f0 = 1 ; f1 = nombre de sommets ; f2 = nombres d'arcs ; f3 = nombre de triangles ; etc.
	...
	/* d-compacité d'un complexe seul, définit dans MCM13  */
	//
	//				 f_(d+1)
	// d-compacité = --------
	//				  (f_1)
	...
	// d-compacité moyenne pour un accord quelconque (formule dans la thèse)
	...
	// Proposition 0 de compacité globale. Approche par décimale : la nième décimale corresond à la n-compacité
	...
	// Proposition 1 de compacité globale
//	public float get_complex_global_compactness(){
...
//	}
	...
	public HashSet<PitchClassMappingList> get_inclusion_pc_mapping_set(ChordComplex complex){ // 254.contains(321)
		...
		if (complex.get_dimension()>get_dimension()) return mapping_set;
		...
		System.out.println("sous complexe cherché : "+complex_vertices);
		
		for (ArrayList<Simplex> solution : get_solutions(complex_vertices_count,complex)){
//			if (is_available_injection(candidat, complex.get_dim_Plex_simplices(0), complex)){
				solution_list.add(solution);
				System.out.println("candidat trouvé : "+solution);
...
//			}
	...
	public ArrayList<ArrayList<Simplex>> get_solutions(int size, ChordComplex complex){
		...
		for (int i=0;i<size;i++) new_list.add(null);
		recur2(new_list,0,vertices,seqs,complex);
		return seqs;
	}
	
	public void recur2(ArrayList<Simplex> list, int index, ArrayList<Simplex> vertices, ArrayList<ArrayList<Simplex>> seqs, ChordComplex complex){
	...
	public boolean is_same_complex_structure(ChordComplex complex){
		...
		if (get_dimension() != complex.get_dimension()) return false;
		int dim = get_dimension();
		for (int d = 0;d<=dim;d++){
			if (get_d_simplex_count(d) != complex.get_d_simplex_count(d)) return false;
		}
		for (int d = 0;d<=get_dimension();d++) if (get_betti_number(d) != complex.get_betti_number(d)) return false;
		...
		for (ArrayList<Simplex> candidate : sk1_solution_list){
			if (is_available_bijection(vertices_a, candidate, complex)) real_solution_list.add(candidate);
		}
			
		if (real_solution_list.size()!=0){
			//System.out.println("REAL SOLUTIONS : ");	
			//for (ArrayList<Simplex> solution : real_solution_list) System.out.println(solution);
			return true;
		}
	...
	public void recur(ChordComplex complex, ArrayList<Simplex> vertices_a, ArrayList<Simplex> vertices_b, ArrayList<Simplex> new_list, ArrayList<ArrayList<Simplex>> sk1_solution_list, int index){
	...
	public boolean is_available_bijection(ArrayList<Simplex> vertices_a, ArrayList<Simplex> vertices_b, ChordComplex complex){
		...
		for (int d=1;d<=dimension;d++){
			for (Simplex simplex : get_dim_Plex_simplices(d)){
				...
				for (Simplex vertex : incident_vertices) index_a_list.add(vertices_a.indexOf(vertex));					
				ArrayList<Simplex> corresponding_b_vertices = new ArrayList<Simplex>();
				for (int index : index_a_list) corresponding_b_vertices.add(vertices_b.get(index));
				if (!complex.contains_simplex_exactly_surrounded_by_vertices(corresponding_b_vertices)) return false;
			}
		}
		return true;
	}
	
	public boolean is_available_injection(ArrayList<Simplex> vertices_a, ArrayList<Simplex> vertices_b, ChordComplex complex){
		...
		for (int d=1;d<=dimension;d++){
			for (Simplex simplex : complex.get_dim_Plex_simplices(d)){
				...
				for (Simplex vertex : incident_vertices) index_b_list.add(vertices_b.indexOf(vertex));					
				ArrayList<Simplex> corresponding_a_vertices = new ArrayList<Simplex>();
				for (int index : index_b_list) corresponding_a_vertices.add(vertices_a.get(index));
				if (!contains_simplex_exactly_surrounded_by_vertices(corresponding_a_vertices)) return false;
			}
		}
		return true;
	}
	...
	public BarcodeCollection<java.lang.Integer> get_bar_code(){
		...
		AbstractPersistenceAlgorithm<Simplex> persistence = Plex4.getModularSimplicialAlgorithm(get_dimension()+1,HOMOLOGY_COEF);
		//BarcodeCollection<java.lang.Double> bar_code = persistence.computeIntervals(complex);
	...
	public void verif(ArrayList<Integer> betti_list){
		...
		if (alt_sum_betti != get_euler_characteristic()) {
			System.err.println("Incohérence Betti : "+betti_list+" et Euler : "+get_euler_characteristic());
		}
	}
	...
	public ArrayList<Simplex> get_Plex_simplices(){
		...
		while(i.hasNext()) simplices.add(i.next());
		return simplices;
	}
	...
	public ArrayList<Simplex> get_dim_Plex_simplices(int dim){
		...
		while(i.hasNext()){
			simplex = i.next();
			if (simplex.getDimension()==dim) dim_simplices.add(simplex);
		}
	...
	public ArrayList<Simplex> get_higher_neighbors(Simplex simplex){
		...
		for (int v : simplex.getVertices()) simplex_vertices.add(v);
		for (Simplex s : get_Plex_simplices()){
			ArrayList<Integer> s_vertices = new ArrayList<Integer>();
			for (int v : s.getVertices()) s_vertices.add(v);
			if (s_vertices.containsAll(simplex_vertices) && simplex != s) higher_neighbors.add(s);
		}
	...
	public ArrayList<Simplex> get_lower_neighbors(Simplex simplex){
		...
		for (Simplex s : get_Plex_simplices()){
			ArrayList<Integer> s_vertices = new ArrayList<Integer>();
			for (int v : s.getVertices()) s_vertices.add(v);
			if (simplex_vertices.containsAll(s_vertices) && simplex != s) lower_neighbors.add(s);
		}
	...
	// returns neighbors (faces, faces of faces, cofaces, cofaces of cofaces, etc.) of dimension p
	public ArrayList<Simplex> get_p_neighbors(Simplex simplex, int p){
		ArrayList<Simplex> p_neighbors = new ArrayList<Simplex>();
		if (simplex.getDimension()==p) return p_neighbors; 
		ArrayList<Integer> simplex_vertices = new ArrayList<Integer>();
		for (int v : simplex.getVertices()) simplex_vertices.add(v);
		for (Simplex s : get_Plex_simplices()){
			if (s.getDimension()==p){
				...
				if (p>simplex.getDimension()){
					if (s_vertices.containsAll(simplex_vertices)) p_neighbors.add(s);
				} else {
					if (simplex_vertices.containsAll(s_vertices)) p_neighbors.add(s);
				}
			}
		}
		return p_neighbors;
	...
	// Two d-cells are (d,p)-neighbor if they have a common border of dimension p when p ≤ n or if they are in the boundary of a p-cell of higher dimension.
	public ArrayList<Simplex> get_d_p_neighbors(Simplex simplex, int p){
		...
		for (Simplex p_neighbor : p_neighbors){
			for (Simplex d_p_neighbor : get_p_neighbors(p_neighbor, d)){
				if (d_p_neighbor != simplex) d_p_neighbors.add(d_p_neighbor);
			}
		}
	...
	public ArrayList<Simplex> get_cofaces(Simplex simplex){
		...
		for (Simplex higher_simplex : just_higher_simplices){
			if (get_faces(higher_simplex).contains(simplex)) cofaces.add(higher_simplex);
		}
		return cofaces;
	}
	
//	public boolean is_a_surface(){
//		if ((get_dimension() !=2) || (get_connected_componnent_count() != 1)) return false;
...
//		if ((get_dimension() !=3) || (get_connected_componnent_count() != 1)) return false;
...
//	}
	...
	// Ne veut rien dire pour l'instant (Il n'existe pas de terme pour qualifier cette propriété) 
...
//	}
	...
	// The d-complex D is strongly connected if D satisfies the following condition : for any d-simplex I and I', there is a sequence of d-simplex I = I1, I2, ... , Is = I' tel que dim(Ii inter Ii+1)=d-1 pour tout 1<=i<=s-1 
	public boolean is_strongly_connected(){
		
		int dim = get_dimension();
		ArrayList<Simplex> simplices = get_dim_Plex_simplices(dim);
		if (simplices.isEmpty()) return false;
		...
		while(true){
			...
			if (d_simplices.size() == size) return true;
	...
	public boolean is_pseudo_variety(){
		if (!is_strongly_connected()) return false;
		...
		for (Simplex d_simplex : d_simplices) lower_faces.addAll(get_lower_neighbors(d_simplex));
		if (lower_faces.size() != (get_Plex_simplices().size()-d_simplices.size())) return false;
		...
		for (Simplex d_m1_simplex : d_m1_simplices) {
			if (get_cofaces(d_m1_simplex).size()>2) return false;
		}
	...
	public boolean has_boundary(){
		...
		for (Simplex simplex : one_less_dim_simpleces){
			if (get_cofaces(simplex).size() < 2) return true;
		}
		return false;
	}
	...
	public void print_all_simplex(){
		...
		while(i.hasNext()){
			simplex = i.next();
			//System.out.println("simplex : "+simplex+"  boundary : "+Table.toString(simplex.getBoundaryArray()));
			//System.out.println("simplex : "+simplex+"  boundary : "+Table.toString(simplex.getBoundaryArray()));
			//System.out.println("simplex : "+simplex+"  d,1-neighbors : "+get_d_p_neighbors(simplex,1));
		}
	}
