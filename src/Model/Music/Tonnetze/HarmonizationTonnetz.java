...
class NVoiceTransition extends ArrayList<PitchTransition> {
	...
	public NVoiceTransition(ArrayList<Integer> pitch_list1, ArrayList<Integer> pitch_list2) {
		...
		if (pitch_list1.size() != pitch_list2.size()) {
			System.out.println("sets are not the same size : " + PitchSet.print_pitch_list(pitch_list1) + " "
					+ PitchSet.print_pitch_list(pitch_list2));

			// System.out.println("sets are not the same size : "+pitch_list1+"
			// "+pitch_list2+". Complete the shortest one with an extra lower pitch");
			// assert false;
		...
		assert pitch_list1.size() == pitch_list2.size()
				: "sets should now be the same size : " + pitch_list1 + " " + pitch_list2;
		_list1 = pitch_list1;
		_list2 = pitch_list2;
	...
	public float get_compliance(HarmonizationTonnetz harmo_tonnetz) {
		...
		if (size() == 1)
			return harmo_tonnetz.get_horizontal_compliance(get(0)._p1, get(0)._p2);
		...
		// Si aucune plaque n'est 0, alors on dfinit la compliance comme le produit
		// VC1*HC*VC2
		// Chaque VC est pris 2 fois en compte car on considre un accord une fois dans
		// le contexte de son pass
		// et une fois dans le contexte de son futur

		// System.out.println(" ... horizontal compliance :
		// "+get_horizontal_compliance(harmo_tonnetz));
		// System.out.println(vc1);
		// System.out.println(vc2);

		// Finalement on ne prend pas en compte les compl verticales car elles seront
		// prises en compte lors du produit final
	...
	// horizontal compliance = moyenne des horizontal compliances de chaque voice
	// transition
...
class TwoVoiceTransition {
	...
	// // compliance de la plaque comme produit des 4 compliances : inconvnient : si
	// une compliance
	// // verticale est nulle la compliance de la plaque est nulle
	// //return vc1*vc2*hc1*hc2;
	// // 2me mthode : on fait la moyenne des 4 compliances : inconvnient : on ne
	// prend plus en compte
	// // les 2-compliances verticales >0 (seules les arrtes verticales comptent)
	// // 3me mthode : produit vc1*hc*vc2
...
public class HarmonizationTonnetz {
	...
	// public PitchRanking get_pitch_ranking(HarmonizationVoice voice,ColStream
	// other_voices,long key,PitchRanking vertical_ranking){
	//
	...
	// if(_h_tonnetz_ranking.get(coef).get_neighbors_pitch(last_pitch).contains(pitch)
	// || pitch==last_pitch){
	// float plate_comp =
	// get_transversal_compliance(other_voices.get(voice.lowerKey(key)),voice.lowerEntry(key).getValue().get_pitch(),
	// other_voices.get(key),pitch);
	...
	public HashSet<PitchWithCompliance> get_pitch_with_compliance_set(PitchRanking vertical_ranking,
			HarmonizationVoice voice, long key, PitchSetStream other_voices) {
		...
		for (float vertical_score : vertical_ranking.keySet()) {
			for (int pitch : vertical_ranking.get(vertical_score)) {
				...
				if (other_voices.lowerEntry(key) != null && !other_voices.lowerEntry(key).getValue().isEmpty()) {
					PitchSetWithDuration last_other_voices_col = new PitchSetWithDuration(
							other_voices.lowerEntry(key).getValue());
					transversal_compliance = get_transversal_compliance(last_other_voices_col, last_pitch,
							other_voices.floorEntry(key).getValue(), pitch);
				} else {
					transversal_compliance = 1;
				}
				pitch_with_compliance_set.add(new PitchWithCompliance(pitch, vertical_score,
						get_horizontal_compliance(last_pitch, pitch), transversal_compliance));
			}
		}
