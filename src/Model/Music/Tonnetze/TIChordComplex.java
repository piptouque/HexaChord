...
public class TIChordComplex extends TComplex {
	...
	public TIChordComplex(STIntervallicStructure intervallic_structure) {
		...
		// System.out.println(get_latex_name()+" "+get_betti_numbers_list()+"
		// "+get_betty_numbers()+"\n"+get_dimension());
	...
	public float get_global_compliance(PitchSet pitch_set) {
		...
		if (pitch_set.to_PitchClassSet().size() == 0)
			return 0;
		if (pitch_set.to_PitchClassSet().size() == 1)
			return 1; // Une pitch class seule a une compliance de 1 dans un espace qui la contient

		// Choix arbitraire de calcul pour la compliance globale. Ici, on choisit C =
		// 3*(2_c) + 1_c
		// Calcul tablit pour garder une hirarchie cohrente de la compliance des accords
		// tonaux dans le tonnetz 345

		// return (3*get_2_compliance(pitch_set)+get_1_compliance(pitch_set))/4;

		// Autre mthode : mthode de Jean-Louis :
		// 2_c + (1-1/(1+1_c)) On normalise en diviasant par 1,5 car c'est la valeur max
		// que ca peut atteindre (1_comp et 2_comp = 1)

		// return
		// (get_2_compliance(pitch_set)+(1-(1/(1+get_1_compliance(pitch_set)))))/(float)1.5;
	...
	// ArrayList<Integer> pc_list = new
	// ArrayList<Integer>(pitch_set.to_PitchClassSet());
	...
	// PitchClassSet three_pc_set = new
	// PitchClassSet(pc_list.get(i),pc_list.get(j),pc_list.get(k));
	...
	// PitchClassSet four_pc_set = new
	// PitchClassSet(pc_list.get(i),pc_list.get(j),pc_list.get(k),pc_list.get(m));
	...
	public float get_absolute_compliance(PitchSet pitch_set) {
		if (_intervallic_structure.get_N() == 12) {
			...
			if (!_folded_graph_tonnetz.get_scale().containsAll(pitch_set.to_PitchClassSet()))
				return 0;
			if (pitch_set.to_N_pc_set(_folded_graph_tonnetz.get_scale()).get_intervallic_structure()
					.equals(_intervallic_structure)) {
				return 1;
			}
		}
		return 0;
	...
	public PitchClassRanking get_compliance_ranking(PitchSet pitch_set, PitchClassSet forced_chord) {
		...
		for (Integer i : _folded_graph_tonnetz.get_scale()) {
			if (forced_chord.contains(i)) {
				...
				pc2.add(i);
				ranking.add(i, get_global_compliance(pc2));
				// System.out.println("mc : "+mc+" pitch : "+i+" sum_compliance :
				// "+get_sum_compliance(mc2));
			}
		}
		return ranking;
	}
	...
	public boolean includes_chord_simplex(PitchClassSet pc_set) {
		...
		if (_intervallic_structure.get_N() != 12) {
			if (!_folded_graph_tonnetz.get_scale().containsAll(pc_set))
				return false;
		...
		for (int i : PitchClassSet.IFUNC_vector(complex_prime_chord, N_pc_set, _intervallic_structure.get_N())) {
			if (i == pc_set_cardinality)
				return true;
		}
		for (int i : PitchClassSet.IFUNC_vector(complex_prime_chord, N_pc_set.inversion(0),
				_intervallic_structure.get_N())) {
			if (i == pc_set_cardinality)
				return true;
		}
		return false;
	...
	public static ArrayList<TIChordComplex> getZ12TonnetzChromaticChordComplexList() {
		if (_z12TonnetzChordComplexList == null) {
			...
			for (STIntervallicStructure si : si_list)
				_z12TonnetzChordComplexList.add(new TIChordComplex(si));
		}
		return _z12TonnetzChordComplexList;
	}

	/* ------ Les 12 complexes chromatiques constitus d'accords de taille 3------ */

	// private static ArrayList<TonnetzChordComplex> _z12Tonnetz_3_ChordComplexList;

	// public static ArrayList<TonnetzChordComplex>
	// getZ12Tonnetz_3_ChordComplexList() {
	// if (_z12Tonnetz_3_ChordComplexList == null) {
	// ArrayList<STIntervallicStructure> si_list =
	// STIntervallicStructure.enum_SI_up_to_flip(3,12);
	// _z12Tonnetz_3_ChordComplexList = new ArrayList<TonnetzChordComplex>();
	// for (STIntervallicStructure si : si_list)
	// _z12Tonnetz_3_ChordComplexList.add(new TonnetzChordComplex(si));
	// System.out.println("Et voici les Triangular complexes :
	// "+_z12Tonnetz_3_ChordComplexList);
	// }
	// return _z12Tonnetz_3_ChordComplexList;
	// }
	//
	// public static int getZ12Tonnetz_3_ChordComplexList_index(TonnetzChordComplex
	// h) {
	// assert getZ12Tonnetz_3_ChordComplexList().indexOf(h) != -1 : "3_ChordComplex
	// not found";
	// return getZ12Tonnetz_3_ChordComplexList().indexOf(h);
	// }

	/* ------ Les complexes chromatiques constitus d'accords de taille n------ */
	/*
	 * ------ Optimisation : les accords les plus utiliss (de taille 3 et 4) sont
	 * sauvegards ------
	 */
	...
	public static ArrayList<TIChordComplex> getZ12Tonnetz_n_ChordComplexList(int chord_size) {
		...
		if (chord_size == 3) {
			if (_z12Tonnetz_3_ChordComplexList == null) {
				...
				for (STIntervallicStructure si : STIntervallicStructure.enum_SI_up_to_flip(3, 12))
					_z12Tonnetz_3_ChordComplexList.add(new TIChordComplex(si));
			}
			return _z12Tonnetz_3_ChordComplexList;
		}
		...
		if (chord_size == 4) {
			if (_z12Tonnetz_4_ChordComplexList == null) {
				...
				for (STIntervallicStructure si : STIntervallicStructure.enum_SI_up_to_flip(4, 12))
					_z12Tonnetz_4_ChordComplexList.add(new TIChordComplex(si));
			}
		...
		for (STIntervallicStructure si : STIntervallicStructure.enum_SI_up_to_flip(chord_size, 12))
			_z12Tonnetz_n_ChordComplexList.add(new TIChordComplex(si));
	...
	/*
	 * ------ Les 24 complexes heptatoniques de la forme [1,2,4] (12 majeurs et 12
	 * mineurs) ------
	 */
	...
	public static ArrayList<TIChordComplex> getZ7Tonnetz_124_ChordComplexList() {
		if (_z7Tonnetz_124_chordComplexList == null) {
			...
			for (int i = 0; i < 12; i++) {
				_z7Tonnetz_124_chordComplexList
						.add(new TIChordComplex(_z7_intervallic_structure, new Scale(i, "major")));
				_z7Tonnetz_124_chordComplexList
						.add(new TIChordComplex(_z7_intervallic_structure, new Scale(i, "minor")));
			}
		}
	...
	/*
	 * ------ Les 24 complexes heptatoniques de la forme [2,2,3] (mbius strips) (12
	 * majeurs et 12 mineurs) ------
	 */
	...
	public static ArrayList<TIChordComplex> getZ7Tonnetz_223_ChordComplexList() {
		if (_z7Tonnetz_223_chordComplexList == null) {
			...
			for (int i = 0; i < 12; i++) {
				_z7Tonnetz_223_chordComplexList
						.add(new TIChordComplex(_z7_intervallic_structure, new Scale(i, "major")));
				_z7Tonnetz_223_chordComplexList
						.add(new TIChordComplex(_z7_intervallic_structure, new Scale(i, "minor")));
			}
	...
	public static ArrayList<String> get_complex_string_list(ArrayList<TIChordComplex> complex_list) {
		...
		for (TIChordComplex complex : complex_list)
			string_list.add(complex.toString());
		return string_list;
	}

	public String get_latex_name() {
		// return
		// "\\cti{"+_intervallic_structure.elements_to_string_without_space(false)+"}";
	...
	public HashSet<TComplex> get_vpl_neighbor_complexes(HashSet<TComplex> complex_set) {
		...
		for (TComplex complex : complex_set) {
			if (complex.get_STIntervallicStructure().is_contained_up_to_flipSI(neighbor_is_list))
				neighbors_set.add(complex);
		}
