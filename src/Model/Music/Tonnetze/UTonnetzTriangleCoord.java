...
public class UTonnetzTriangleCoord extends UTonnetzCoordList implements Comparable<UTonnetzTriangleCoord> {
	...
	public UTonnetzTriangleCoord(Collection<UTonnetzVertexCoord> tonnetz_coord_list) {
		super();
		assert tonnetz_coord_list.size() == 3
				: "not a triangle because it has " + tonnetz_coord_list.size() + " vertices";
		for (UTonnetzVertexCoord tonnetz_coord : tonnetz_coord_list) {
			add(new UTonnetzVertexCoord(tonnetz_coord));
		}
	}

	public UTonnetzTriangleCoord(UTonnetzVertexCoord vertex1, UTonnetzVertexCoord vertex2,
			UTonnetzVertexCoord vertex3) {
		super();
		add(vertex1);
		add(vertex2);
		add(vertex3);
	...
	// retourne les triangles voisins par un arc y compris le triangle lui mme
	...
	// retourne les triangles voisins par un sommet y compris le triangle lui mme
