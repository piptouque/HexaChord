...
public class UTonnetzCoordList extends ArrayList<UTonnetzVertexCoord> {
	...
	public static int neighborhood_count(UTonnetzCoordList coord_list, UTonnetzVertexCoord coord) {
		...
		for (UTonnetzVertexCoord c : coord_list) {
			if (coord.is_neighbor(c))
				count++;
		}
		return count;
	}

	// return the list of the coords of coord_list which have the more neighborhood
	...
	// public void recur(int index, int n,ArrayList<UTonnetzVertexCoord>
	// list,HashSet<UTonnetzCoordList> set){
	// if (index >= n) {
