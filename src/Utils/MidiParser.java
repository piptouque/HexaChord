...
//	static ProgressDialog _progress_frame;
	...
	// A utiliser sans interface graphique : pas de bar de progression, fonction bloquante
	
	public static PitchSetStream stream_generator(Sequence midi_seq, ArrayList<Integer> track_list, boolean sustain){
//		System.out.println("Voici le nombre de Tracks : "+midi_seq.getTracks().length);
...
//			System.out.println("prog : "+(100*(float)t/(float)midi_seq.getTracks().length)/(float)2);
		}
		PitchSetStream stream = PitchSetStream.mix_colStream_list(track_stream_list);
//		System.out.println("prog : "+75);
		//System.out.println("PPQ = "+Sequence.PPQ+" Resolution = "+midi_seq.getResolution());
		boolean bool = false;
		while(!bool){
			if (!stream.try_defragmentation()) bool = true;
		}
	...
	// - g�n�re le stream dans un thread s�par� gr�ce � un SwingWorker. La fonction n'est donc pas bloquante
	// - affiche une bar de progression
	// - ne retourne rien (le stream est renvoy� � hexachord dans la m�thode done()
	
	public static void stream_generator_thread(Sequence midi_seq, ArrayList<Integer> track_list, boolean sustain, int round_tick){
...
//			System.out.println("voici les tracks : "+track_list);
			...
			boolean bool = false;
			while(!bool){
				if (!_stream.try_defragmentation()) bool = true;
			}
			return _stream;
		}
		...
		public void done(){            
			if(SwingUtilities.isEventDispatchThread())
				try {
					HexaChord.getInstance().stream_init(get(),round_tick);	// get() fournit le stream renvoy� par doInBackground
	...
	public static PitchSetStream stream_generator_without_track(Sequence midi_seq, int track_to_ignore, boolean sustain){
		ArrayList<Integer> track_list = new ArrayList<Integer>();
		for (int t=0;t<midi_seq.getTracks().length;t++){
			if (t != track_to_ignore) track_list.add(t);
		}
		return stream_generator(midi_seq, track_list, sustain);
	}

	public static PitchSetStream stream_generator_with_one_track(Sequence midi_seq, int track_to_parse, boolean sustain){
		ArrayList<Integer> track_list = new ArrayList<Integer>();
		for (int t=0;t<midi_seq.getTracks().length;t++){
			if (t == track_to_parse) track_list.add(t);
		}
		return stream_generator(midi_seq, track_list, sustain);
	}
	...
	public static PitchSetStream track_to_colStream(Track track, ArrayList<TreeMap<Long,Boolean>> sustain_table){
		...
		HashSet<Integer> track_channels = new HashSet<Integer>();	// Ensembles des cannaux sollicit�s par le fichier Midi
		ArrayList<ArrayList<MidiEvent>> channels = new ArrayList<ArrayList<MidiEvent>>();	// Les 16 listes d'�v�nement associ�es aux 16 canaux  
		for (int i = 0;i<16;i++){
...
//		System.out.println("Track size : "+t.size());
		for (int n=0; n<track.size();n++){
			...
			if (track.get(n).getMessage() instanceof ShortMessage){
				//System.out.println(t.get(n).getTick()+" : MidiEvent : "+MessageInfo.toString(t.get(n).getMessage()));
				ShortMessage short_message = (ShortMessage) track.get(n).getMessage();
				track_channels.add(short_message.getChannel());	// on met � jour l'ensemble des cannaux sollicit�s
				channels.get(short_message.getChannel()).add(track.get(n)); // on rajoute l'�v�nement dans la liste associ� au canal sollicit�
			}	
		}
...
//		System.out.println("Les track_ticks : "+ticks_list);
		...
		ArrayList<Integer> channels_list = new ArrayList<Integer>(track_channels); // Liste des canaux sollicit�s par la piste
...
//		System.out.println("Les channels : "+channels);
		...
		for (int ch = 0;ch<channels.size();ch++){
			if (channels.get(ch).size()>0 && ch!=9){	// On ne consid�re pas les pitchs inclus dans la channel d�di�e aux percussions
				//System.out.println("Channel "+ch);
				if (last_tick < sustain_table.get(ch).lastKey()) last_tick = sustain_table.get(ch).lastKey();	// le dernier tick peut �tre un sustain 
				PitchSetStream channel_stream = midi_messages_to_colStream(channels.get(ch), last_tick, sustain_table.get(ch));
				//ColStream channel_stream = midi_messages_to_colStream(channels.get(ch), last_tick, sustain_list);
	...
	public static PitchSetStream midi_messages_to_colStream(ArrayList<MidiEvent> midi_event_list, long last_tick, TreeMap<Long,Boolean> sustain_tree){
//		for (int n=0;n<list.size();n++){
//			System.out.println(list.get(n).getTick()+" "+MessageInfo.toString(list.get(n).getMessage()));
//		}
		...
		for (MidiEvent e : midi_event_list) {
			...
			if (e.getTick() > higher) higher =e.getTick(); 
		}
		...
		for (Long l : sustain_tree.keySet()) {
			if (bool){
				if (!sustain_tree.get(l)){
					...
					if (l>higher) bool = false;  // on s'arr�te lorsqu'il n'y a plus de NOTE ON et NOTE OFF
				}
			}
		}
...
//		System.out.println("Voici la liste des Ticks : "+channel_ticks);
		...
		for (int n=0;n<channel_all_event_ticks.size();n++){
			if (n==0){	// Premier Event de la s�quence
				if (channel_all_event_ticks.size()>1){
					PitchSetWithDuration collection = new PitchSetWithDuration(channel_all_event_ticks.get(1)-channel_all_event_ticks.get(0));
					for (MidiEvent e : midi_event_list){
			...
			// Autre events que le premier ou le dernier
				...
				if (channel_all_event_ticks.get(n)!=last_tick){
						...
						if (sustain_tree.containsKey(channel_all_event_ticks.get(n))){
							if (!sustain_tree.get(channel_all_event_ticks.get(n))){ // Si cet instant correspond � un relachement de p�dale
						...
						for (MidiEvent e : midi_event_list){
							if (e.getTick()==channel_all_event_ticks.get(n)){
								ShortMessage short_message = (ShortMessage) e.getMessage();
								if (short_message.getCommand()==ShortMessage.NOTE_OFF || (short_message.getCommand()==ShortMessage.NOTE_ON && short_message.getData2() == 0)){
						...
						// 3) NOTE ON ?  Il est n�c�ssaire de faire 2 boucles for � la suite sinon les NOTE_OFF annulent imm�diatement les NOTES ON
						for (MidiEvent e : midi_event_list){
							if (e.getTick()==channel_all_event_ticks.get(n)){
								ShortMessage short_message = (ShortMessage) e.getMessage();
								if (short_message.getCommand()==ShortMessage.NOTE_ON && short_message.getData2() != 0){
		...
		//System.out.println("Voci la col stream de la Channel de taille "+col_stream.size()+" : "+col_stream);
		return col_stream;		
	}
	
	private static ArrayList<TreeMap<Long,Boolean>> build_sustain_table(Sequence midi_seq, boolean sustain){
		
		ArrayList<TreeMap<Long,Boolean>> table = new ArrayList<TreeMap<Long,Boolean>>(); // Chaque canal est repr�sent� par un TreeMap
		for (int n = 0;n<16;n++){
			table.add(new TreeMap<Long,Boolean>());
			table.get(n).put((long) 0, false);
		}
		if (!sustain) {	// si on ne prend pas en compte le sustain, la table indique false dans le premier et unique Entry de chacun des canaux
			return table;
		}
		...
		for (int i=0;i<midi_seq.getTracks().length;i++){
			...
			for (int n=0; n<t.size();n++){
				...
				if (message instanceof ShortMessage && ((ShortMessage) message).getCommand()==ShortMessage.CONTROL_CHANGE && ((ShortMessage) message).getData1() == 64){
					//System.out.println(t.get(n).getTick()+" : MidiEvent : "+MessageInfo.toString(t.get(n).getMessage()));
					table.get(((ShortMessage) message).getChannel()).put(t.get(n).getTick(), ((ShortMessage)message).getData2()>63 ? true : false);
				}	
			}
		}
		return table;
  	...
  	public static List<Integer> get_NOTE_ON_pitch_list_from_track(Track track){
		...
		for (int p : pitch_event_sequence) if (p!=-1) NOTE_ON_pitch_sequence.add(p);
			...
			for (int j = 0;j<track.size();j++){
				...
				if (midi_message instanceof MetaMessage){
					...
					if (meta_message.getType()==0x58){
						...
						numerator = data[0];
						//System.out.println("meta message type : "+meta_message.getType()+" data : "+Table.toString(data));	
					}
				}
			}
		}
  	...
  	public static MidiMessage get_onset_message(Track track, Long tick){
  		...
  		for(int i=0;i<track.size();i++){
  			if (track.get(i).getTick()==tick && track.get(i).getMessage() instanceof ShortMessage && ((ShortMessage) track.get(i).getMessage()).getCommand()==ShortMessage.NOTE_ON){
