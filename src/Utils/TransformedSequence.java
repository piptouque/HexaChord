...
public class TransformedSequence extends Sequence {
	...
	// public TransformedSequence(Sequence origine_seq,float arg0, int arg1) throws
	...
	// getTracks()[tr].add(new MidiEvent((MidiMessage)
	// origine_message.clone(),origine_track.get(ev).getTick()));
	// }
	// }
	// }
	...
	public static TransformedSequence transforme_with_pc_mapping(Sequence origine_sequence,
			PitchClassMappingTree pitch_class_mapping, String name) {
		...
		for (int i = 0; i < origine_sequence.getTracks().length; i++) {
			...
			for (int j = 0; j < original_track.size(); j++) {
				...
				if (original_message instanceof ShortMessage && (((ShortMessage) original_message).getChannel() != 9)) {
					...
					if (((ShortMessage) original_message).getCommand() == ShortMessage.NOTE_ON
							&& ((ShortMessage) original_message).getData2() != 0) {
						...
						if (new_pitch_class != -1) {
							...
							try {
								new_message.setMessage(ShortMessage.NOTE_ON,
										((ShortMessage) original_message).getChannel(), new_pitch,
							...
							// NOTE ON ignors soit car absentes de l'espace de dpart
					...
					if (((ShortMessage) original_message).getCommand() == ShortMessage.NOTE_OFF
						...
						// assert playing_pitch_to_remove != null : "cant turn OFF original pitch
						// "+original_pitch+" because it is not played";
		...
		if (ignored_NOTE_ON_events > 0)
			System.err.println("failed to map " + ignored_NOTE_ON_events + " NOTE ON");
		return new_sequence;
	}
	...
	public static int pitch_class_to_pitch(int pitch_class, int original_pitch) {
		...
		// System.err.println("ERROR ! pitch class : "+pitch_class+" original_pitch :
		// "+original_pitch);
		return 0;
	}
