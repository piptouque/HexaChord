...
public abstract class SequenceTransformation {
			...
			for (int j = 0; j < track.size(); j++) {
				...
				if (message instanceof ShortMessage) {
					if (((ShortMessage) message).getCommand() == ShortMessage.NOTE_ON
						...
						try {
							// diffrentes transfos possibles
							// new_mess.setMessage(((ShortMessage) message).getCommand(), ((ShortMessage)
							// message).getChannel(), transfo_pitch(((ShortMessage) message).getData1()),
							// ((ShortMessage) message).getData2());
							// new_mess.setMessage(((ShortMessage) message).getCommand(), ((ShortMessage)
							// message).getChannel(), transfo_pitch(((ShortMessage) message).getData1()),
							// ((ShortMessage) message).getData2());
							new_mess.setMessage(((ShortMessage) message).getCommand(),
									((ShortMessage) message).getChannel(),
									inversion_pitch(((ShortMessage) message).getData1(), 0),
									((ShortMessage) message).getData2());
	...
	static public Sequence inverted_path(Sequence sequence, PitchSetStream stream, SquareGridCoordPath new_path,
			PlanarUnfoldedTonnetz tonnetz) {
		Sequence new_seq = new_sequence(sequence);
		...
		for (int i = 0; i < sequence.getTracks().length; i++) {
			...
			// Tampon dans lequel demeurent les pitchs entre les dates NOTE_ON et NOTE_OFF
			// [pitch initial|pitch transform]
			...
			for (int j = 0; j < track.size(); j++) {
				...
				if (message instanceof ShortMessage) {
					...
					if (((ShortMessage) message).getCommand() == ShortMessage.NOTE_ON
							|| ((ShortMessage) message).getCommand() == ShortMessage.NOTE_OFF) {
						ShortMessage new_mess = new ShortMessage();
						if (((ShortMessage) message).getCommand() == ShortMessage.NOTE_OFF
								|| (((ShortMessage) message).getData2() == 0)) {
							for (int[] p : pitch_playing) {
								// System.out.println("p[0]:"+p[0]+" pitch : "+((ShortMessage)
								// message).getData1());
								if (p[0] == ((ShortMessage) message).getData1()) {
									...
									try {
										// diffrentes transfos possibles
										// new_mess.setMessage(((ShortMessage) message).getCommand(), ((ShortMessage)
										// message).getChannel(), inversion_pitch(((ShortMessage)
										// message).getData1(),0), ((ShortMessage) message).getData2());
										new_mess.setMessage(((ShortMessage) message).getCommand(),
												((ShortMessage) message).getChannel(), p[1],
												((ShortMessage) message).getData2());
									} catch (InvalidMidiDataException e) {
										// TODO Auto-generated catch block
										e.printStackTrace();
							...
							try {
								...
								// message).getData1(),0), ((ShortMessage) message).getData2());
								new_mess.setMessage(((ShortMessage) message).getCommand(),
										((ShortMessage) message).getChannel(), new_pitch,
										((ShortMessage) message).getData2());
							} catch (InvalidMidiDataException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
			...
			for (int j = 0; j < original_track.size(); j++) {
				...
				if (original_message instanceof ShortMessage) {
					if (((ShortMessage) original_message).getCommand() == ShortMessage.NOTE_ON) {
						...
						// System.out.println("original pitch : "+((ShortMessage)
						// original_message).getData1());
						int new_pitch = path_transformation.get_new_pitch(original_pitch, tick);
						if (new_pitch != 0) { // new_pitch = 0 si le pitch n'a pas t trouv cette position (pas normal,
												// erreur trouver)
						...
						try {
							new_message.setMessage(ShortMessage.NOTE_ON, ((ShortMessage) original_message).getChannel(),
									new_pitch, ((ShortMessage) original_message).getData2());
						} catch (InvalidMidiDataException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
					...
					if (((ShortMessage) original_message).getCommand() == ShortMessage.NOTE_OFF) {
						...
						assert playing_pitch_to_remove != null
								: "cant turn OFF original pitch " + original_pitch + " because it is not played";
						playing_pitchs.remove(playing_pitch_to_remove);

						ShortMessage new_message = new ShortMessage();
						try {
							new_message.setMessage(ShortMessage.NOTE_OFF,
									((ShortMessage) original_message).getChannel(), pitch_to_turn_off,
									((ShortMessage) original_message).getData2());
						} catch (InvalidMidiDataException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
					...
					if (((ShortMessage) original_message).getCommand() != ShortMessage.NOTE_OFF
							&& ((ShortMessage) original_message).getCommand() != ShortMessage.NOTE_ON) {
